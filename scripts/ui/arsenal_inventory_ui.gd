...
var _charm_style: StyleBoxFlat
var _charm_empty_style: StyleBoxFlat
...
func _ready() -> void:
	...
	_strip_style = _style(Color(0.12,0.045,0.12,0.88), Color(1.0,0.47,0.69,0.48), 12, 2)
	_weapon_style = _style(Color(0.20,0.07,0.18,0.94), Color(1.0,0.46,0.69,0.70), 7, 2)
	_weapon_empty_style = _style(Color(0.20,0.07,0.18,0.42), Color(1.0,0.46,0.69,0.24), 7, 2)
	_charm_style = _style(Color(0.13,0.08,0.22,0.94), Color(0.73,0.58,1.0,0.70), 7, 2)
	_charm_empty_style = _style(Color(0.13,0.08,0.22,0.42), Color(0.73,0.58,1.0,0.24), 7, 2)
	_detail_style = _style(Color(0.075,0.025,0.075,0.965), Color(1.0,0.50,0.72,0.88), 20, 3)
...
func _draw_compact_strip() -> void:
	...
	draw_style_box(_strip_style, Rect2(base + Vector2(-12, -10), Vector2(556, 72)))
	draw_string(_font, base + Vector2(0, -15), "ARSENAL   [I] DETAILS", HORIZONTAL_ALIGNMENT_LEFT, 240, 12, Color(1.0,0.78,0.88,0.88))
	var weapon_ids: Array[String] = []
	for key: Variant in _arsenal.weapons.keys(): weapon_ids.append(String(key))
	var passive_ids: Array[String] = []
	for key: Variant in _arsenal.passives.keys(): passive_ids.append(String(key))
	for i: int in ArsenalCatalog.MAX_WEAPON_SLOTS:
		...
		if occupied:
			...
			_draw_slot_text(pos, String(data.get("short","?")), int(_arsenal.weapons[id]), bool(_arsenal.evolved.get(id,false)))
		else:
	...
	for i: int in ArsenalCatalog.MAX_PASSIVE_SLOTS:
		...
		draw_style_box(_charm_style if occupied else _charm_empty_style, Rect2(pos, Vector2(38,29)))
		if occupied:
...
func _draw_slot_text(pos: Vector2, short_name: String, level_value: int, is_evolved: bool) -> void:
	var main_color: Color = Color(1.0,0.90,0.42,1.0) if is_evolved else Color(1.0,0.88,0.95,1.0)
	...
	if is_evolved: draw_circle(pos + Vector2(33,6), 2.6, Color(1.0,0.88,0.30,0.95))

func _draw_detail_panel() -> void:
	draw_style_box(_detail_style, Rect2(270,128,740,458))
	...
	draw_string(_font, Vector2(300,188), "6 weapons + 6 charms • weapon Lv 8 • charm Lv 5", HORIZONTAL_ALIGNMENT_LEFT, 620, 13, Color(0.92,0.74,0.90,0.78))
	...
	for key: Variant in _arsenal.weapons.keys():
		if row >= ArsenalCatalog.MAX_WEAPON_SLOTS: break
		...
		var passive_data: Dictionary = ArsenalCatalog.get_passive(String(data.get("passive","")))
		draw_string(_font, Vector2(300,y+31), "EVOLVE: %s" % String(passive_data.get("name","?")), HORIZONTAL_ALIGNMENT_LEFT, 330, 10, Color(1.0,0.66,0.80,0.64))
		row += 1
	row = 0
	for key: Variant in _arsenal.passives.keys():
		if row >= ArsenalCatalog.MAX_PASSIVE_SLOTS: break
		var id: String = String(key)
		var data: Dictionary = ArsenalCatalog.get_passive(id)
		var y: float = 258.0 + float(row) * 49.0
