...
func emit_burst(global_origin: Vector2, direction: Vector2, amount: int = 8) -> void:
	...
	for _i: int in amount:
		...
		add_child(droplet)
		var spread: Vector2 = base_dir.rotated(randf_range(-1.45, 1.45))
		var speed: float = randf_range(70.0, 230.0)
		var shade: Color = Color("9f0b2c") if randf() < 0.35 else Color("d51c42")
